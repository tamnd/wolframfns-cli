...
describes what users get. Pick a format with `-o`, or let wolframfns choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
wolframfns <command> -o table   # aligned columns for reading
wolframfns <command> -o jsonl   # one JSON object per line, for piping
wolframfns <command> -o json    # a single JSON array
wolframfns <command> -o csv     # spreadsheet friendly
wolframfns <command> -o tsv     # tab-separated
wolframfns <command> -o url     # just the URL column
wolframfns <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
wolframfns <command> --fields id,title,url
```
...
```bash
wolframfns <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
wolframfns <command>            # a table, because this is a terminal
wolframfns <command> | wc -l    # JSONL, because this is a pipe
```
